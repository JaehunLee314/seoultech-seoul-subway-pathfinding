...
# Load master CSV (assuming station_name, latitude, longitude columns)
master = pd.read_csv('data_preprocessing\dataset\station_location\subway_line5_coords.csv', encoding='utf-8')

# Station dictionaries provided by user
line1_stations = {
    "trunk": [
...
# Flatten station names from dictionaries
all_user_stations = set()
def flatten(d):
    if isinstance(d, dict):
        ...
        for item in d:
            all_user_stations.add(item)

flatten(line1_stations)
flatten(line2_stations)
flatten(line3_stations)
flatten(line4_stations)
flatten(line5_stations)

# Stations in master
master_names = set(master['station_name'])

# Find stations missing in master
missing_in_master = sorted([st for st in all_user_stations if st not in master_names])

# Find stations in master but missing coords
coords_missing = master[master['station_name'].isin(all_user_stations) &
                        (master['latitude'].isnull() | master['longitude'].isnull())][['station_name', 'latitude', 'longitude']]


print(missing_in_master)
print(coords_missing)