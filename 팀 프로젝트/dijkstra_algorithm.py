...
START_STATION_LINE = '3'
START_STATION_NAME = '남부터미널'
END_STATION_NAME = '용두'
...
if __name__ == "__main__":
    ...
    subway_graph_data = build_subway_graph_by_distance(
        STATION_TRAVEL_INFO_SEOUL_METRO_CSV_PATH, # 정확한 변수명 사용
    ...
    if subway_graph_data is None or not subway_graph_data:
        ...
        if not is_start_node_valid_in_graph:
            print(f"오류: 출발역 {start_node_tuple}이(가) 생성된 그래프 데이터에 유효하게 포함되지 않았습니다. CSV데이터, relevant_lines 또는 build_subway_graph 함수 내 project_stations 정의를 확인하세요.")
        else:
            distances_from_start_km, path_predecessors = dijkstra(subway_graph_data, start_node_tuple)
            ...
            if not possible_end_nodes:
                ...
                if actual_end_node is None or min_dist_to_destination == float('inf'):
                    ...
                    print(f"--- {START_STATION_NAME} ({START_STATION_LINE}호선) 에서 {END_STATION_NAME} (도착: {actual_end_node[0]}호선) 까지 최단 *거리* 경로 ---")
                    print(f"총 거리: {min_dist_to_destination:.2f} km (환승 거리: 실제 데이터 또는 기본 {DEFAULT_TRANSFER_DISTANCE_KM}km 적용)")
                    if final_path:
                        print("경로:")
